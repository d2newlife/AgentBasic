```markdown
# Copilot Instructions for AgentBasic
...
- **`tools.py`**: This file is designated for defining custom tools that the AI agent can utilize. These tools can encapsulate specific functionalities, API calls, or data processing logic that the agent needs to interact with the external environment.
...
3.  **Tools**: Custom tools defined in `tools.py` (or potentially external tools like Wikipedia via `wikipedia` and `langchain-community`) will extend the agent's capabilities.
...
-   **Tool Definition**: Custom tools should be defined as functions or classes in `tools.py` and integrated into the Langchain agent.
